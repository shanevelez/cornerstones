...
export default async function handler(req, res) {
  ...
  try {
    const results = { cleaner: 0, guests: 0, ray_alerts: 0, ray_skipped: false };
    ...
    // Only run if today is Wednesday (3)
    if (today.getDay() === 3) {
      ...
      // 5. Trigger Email if 3+ Sunny & Available Days
      if (availableSunnyDays >= 3) {
